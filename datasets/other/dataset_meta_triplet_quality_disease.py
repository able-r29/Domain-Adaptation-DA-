...
def meta_preprocess(metas, use, n_disease):
    #11
    def age():
    ...
    #n_disease
    def disease():
        dst = np.zeros(n_disease, dtype=np.float32)
        dst[metas['LABEL']] = 1
        return dst
...
class Dataset(Dataset):
    def __init__(self, pathes, train, crop_size, dir_img, meta_use, n_disease, **_):
        self.pathes   = pathes
        self.train    = train
    ...
    def __getitem__(self, i:int):
        ...
        meta1  = meta_preprocess(item1, self.meta_use, self.n_disease)
        meta2  = meta_preprocess(item2, self.meta_use, self.n_disease)
