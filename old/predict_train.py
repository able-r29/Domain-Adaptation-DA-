...
def parse_path(dir_result):
    ...
    path_result = os.path.join(os.path.dirname(dir_result), 'analyze', 'predict_train')
...
def main(i_fold, dir_result:str, device:str) -> None:
    ...
    model = model.to(device)
    ys, t = predict(loader_train, model, device, pre)
    p = loader_train.dataset.pathes

    dst = [ys[0], t, p]

    utils.makedirs(os.path.join(path_result, str(i_fold), 'predict_train'))
    save_pickle(os.path.join(path_result, str(i_fold), 'predict_train')+'.pickle', dst)

    save_pickle(os.path.join(path_result, str(i_fold), 'predict_train')+'_appendix.pickle', ys[1:])
