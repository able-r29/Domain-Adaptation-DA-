...
def get_loss_func(alpha, gamma, path_fold, meta_use, n_class, img_num, device, **karg):
    train_num = list(map(int, utils.load_csv(img_num)[0]))
    train_num = torch.tensor(train_num)
    weight = train_num / torch.sum(train_num)
    weight = 1 / weight
...
def get_metrics(alpha, path_fold, meta_use, n_class, img_num, device, **karg):
    mat = class_accuracy(path_fold, meta_use, n_class)
