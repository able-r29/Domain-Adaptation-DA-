...
class Model(Module):
    def __init__(self, n_class, fc_bias, n_meta, n_dim, middle, **kargs):
        ...
        self.fc3     = nn.Linear(middle*2, n_class, bias=fc_bias)
    ...
    def forward(self, x):
        ...
        ya2, yp2 = torch.chunk(y2, chunks=2, dim=0)

        x = dict(ya=ya, yp=yp, za=za, zp=zp, ya2=ya2, yp2=yp2)
        return x
...
def get_loss_func(alpha, gamma, path_fold, meta_use, n_class, **karg):
    ...
    def _func(x, t):
        ...
        mat_y = 1-mat
        for i, row in enumerate(mat_y):
            for j, col in enumerate(row):
                if i == j:
                    mat_y[i,j] = 0

        l_mr_y = semi_hard_loss(ya2, yp2, t, alpha, mat_y)
        return l_cl + gamma*l_mr + l_mr_y

    return _func


def get_metrics(alpha, path_fold, meta_use, n_class, **karg):
    mat = class_accuracy(path_fold, meta_use, n_class)
    ...
    def _shl_y(x, t):
        ...
        l_mr_y = semi_hard_loss(a2, p2, t, alpha, mat_y)
        return l_mr_y
