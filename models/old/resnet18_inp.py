...
class Model(Module):
    ...
    def forward(self, a, p):
        ...
        ha, hp = torch.chunk(h, chunks=2, dim=0)
        a = torch.rand((ha.shape[0], 1), dtype=torch.float32, device=ha.device)
        m = ha*a + hp*(1-a)
...
def updater(model    :nn.Module,
    ...
    def _update(a, p, ta, tp, la, lp, i=None):
        ...
        mid = cross_entropy_loss(m, ta)
        acc = utils.TopN_accuracy(ya, ta, 1)


        loss = cel + mid * params.model.intermidiate.eta \
