...
import params
from models.resnet18_mod import pretrained_resnet18
from .common import Lambda
from .common import MultilossUpdater as updater
...
def format_params():
    ...
    dst = fmt.format(name, usetrained, fc_bias, n_sim, dk)
    return dst
...
class Model(Module):
    def __init__(self):
        ...
        print('resnet18_trd_akn_ifs2 is used')

        n_class = params.dataset.n_class

        self.base    = pretrained_resnet18()
        self.at      = AttentionModule(256, 28)
...
def show_relative_attention():
    ...
    def save_focus(name_base, imgs, focus):
        ...
        for i, (img, a) in enumerate(zip(imgs, focus)):
            c = np.stack(cmap.jet(a, vmin=1, vmax=2), axis=2)
            c = np.uint8(c * 255)
            c = np.asarray(Image.fromarray(c).resize((224,224)), dtype=np.float32) / 255
